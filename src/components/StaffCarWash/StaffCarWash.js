...
const StaffCarWash = ({ onOpenModal }) => {
  ...
  useEffect(() => {
    ...
    const calculateNearestMonday = () => {
      ...
      const formattedDate =
        nearestMonday.toISOString().split("T")[0] + "T00:00:00.000Z";
    ...
    const calculateNearestThursday = () => {
      ...
      const formattedDate =
        nearestThursday.toISOString().split("T")[0] + "T00:00:00.000Z";
      console.log(formattedDate);
      return formattedDate;
    };
  ...
  useEffect(() => {
    const findQuantities = () => {
      const mondayItem = dates
        ? dates.find((item) => item.date === MondayDate)
        : null;
      const thursdayItem = dates
        ? dates.find((item) => item.date === ThursdayDate)
        : null;
  ...
  const mondayBook = () => {
    setSelectedBooking({
      Date: MondayDate,
      Quantity: MondayQuant,
      Day: "Monday",
    });
    console.log(empBook);
    onOpenModal(
      {
        Date: MondayDate.split("T")[0],
        Quantity: MondayQuant,
        Day: "Monday",
        Car_wash: IDmonday,
      },
  ...
  const thursdayBook = () => {
    setSelectedBooking({
      Date: ThursdayDate,
      Quantity: ThursdayQuant,
      Day: "Thursday",
    });
    onOpenModal(
      {
        Date: ThursdayDate.split("T")[0],
        Quantity: ThursdayQuant,
        Day: "Thursday",
        Car_wash: IDthursday,
      },
    ...
    fetch("/api/CarBookings", {
      ...
      .then((data) => {
        ...
        fetch("/api/CarWash", {
          ...
          body: JSON.stringify({
            Car_wash: car.Car_wash,
            Quantity: dayOfWeek === 1 ? MondayQuant + 1 : ThursdayQuant + 1,
          }),
        })
          .then((response) => response.json())
          .then((data) => {
        ...
        <Right>
            ...
            <Text>
              {empBook ? (
                <>
                  ...
                  <p>
                    {" "}
                    Date of your Car Wash booking:
                    <br />
                    {empBook.date.split("T")[0]}
                  </p>
                  <button
                    onClick={() => deletecarwashbooking(empBook.booking_id)}
                  >
                    Cancel
                  </button>
                </>
          ...
          <Booking>
            <Card>
              ...
              <button aria-label="Monday booking" onClick={mondayBook}>
                Book
              </button>
            </Card>
            <Card>
              ...
              <button aria-label="Thursday booking" onClick={thursdayBook}>
                Book
              </button>
            </Card>
          </Booking>
